...
bool CargaEdificiosTxt(tListaEdificios& lista_edificios)
{
	...
	else
	{
		...
		while (codigo != Cent_ArchEdif)
		{
			...
			//Cargar en la lista
			lista_edificios.edificio[ lista_edificios.cont ] = new tEdificio; //Crear dato
...
bool CargaJugadoresTxt(tListaJugadores& lista_jugadores)
{
	bool carga_ok = true;
	ifstream lectura;
	char basura;
	...
	else
	{
		...
		while (strcmp(nombre, Cent_ArchJug) != 0)
		{
			...
			//Cargar en la lista
			if (lista_jugadores.cont == lista_jugadores.capacidad)
...
bool GuardaJugadoresTxt(const tListaJugadores& lista_jugadores)
{
	bool carga_ok = true;
	ofstream escritura;
