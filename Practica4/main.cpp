...
void Op5Admin_NuevoJugador(tListaJugadores& lista_jugadores)
{
	...
	if ( ListaJugadoresLlena(lista_jugadores) ) {
		setColor(light_red);
		cout << "La lista de jugadores esta llena. No se ha podido insertar" << endl;
		setColor(white);		}
	else {
	...
	while (opcion != 0)
	{
		switch (opcion) {
			case 1: BuscarJugador(lista_jugadores, nom_jug, pos_jug);
					VerEdif_Jugador(lista_edificios, lista_jugadores.jugador[pos_jug], white); break;
...
int main()
{
	...
	//Carga de datos
	carga_ok = CargaEdificios(lista_edificios);
	if(!carga_ok) {
	...
	carga_ok = CargaJugadores(lista_jugadores);
	if(!carga_ok) {
	...
	//Guardado de datos
	cout << endl;
	GuardaEdificios(lista_edificios);
	GuardaJugadores(lista_jugadores);
